...
const Circle = (c, numSections, startAngle) => {
  ...
  const rotate = (angle) => {
    ...
    c.style.transition = `transform ${Math.abs(angle) / 66}s ease-in-out`
...
const majorKeys = Array.from(
  document.querySelectorAll('.major li')
)
const minorKeys = Array.from(
  document.querySelectorAll('.minor li')
)
const staffSections = Array.from(
  document.querySelectorAll('.staff li')
)
...
rings.forEach((ring) => {
  ring.forEach((section) => {
    section.addEventListener(
      'click', e => circleOfFifths.center(ring.indexOf(section))
    )
  })
})
